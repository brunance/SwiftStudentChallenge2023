            ...
            VStack {
                ZStack {
                    ...
                    ZStack {
                        Text(getCurrentText())
                            ...
                            .onAppear {
                                ...
                                let timer = Timer.scheduledTimer(withTimeInterval: interval, repeats: true) { _ in
                                    if self.currentTextCount < TextData.gameObjText[self.currentTextIndex].text.count {
                        ...
                        HStack {
                            Button(action: {
                                self.currentTextIndex = (self.currentTextIndex + 1) % TextData.gameObjText.count
                                self.currentTextCount = 0
                                characterAnimation.runAnim(emotion: TextData.gameObjText[currentTextIndex].emotion)
                            }){
                                Image("textdone")
                                    .resizable()
                ...
                NavigationLink("", destination:  PhysicsOneView(), isActive: $newView)
            }
        }
    }

    func getCurrentText() -> String {
        let currentText = TextData.gameObjText[self.currentTextIndex].text
